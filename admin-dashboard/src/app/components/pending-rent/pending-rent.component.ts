...
export class PendingRentComponent implements OnInit {
  ...
  loadPendingRentData(): void {
    this.loading = true;
    this.dashboardService.getPendingRent().subscribe({
      next: (data) => {
      ...
      error: (error) => {
        console.error('Failed to load pending rent data:', error);
        this.snackBar.open('Failed to load pending rent data', 'Close', { duration: 3000 });
        this.loading = false;
      }
    });
  }
